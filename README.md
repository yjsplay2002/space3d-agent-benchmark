...
| 모델 | 시간 | 생성 토큰 | 총 토큰(캐시 포함) | 자동검증 | 실제 렌더링 | 품질 |
|---|---|---|---|---|---|---|
| 🥇 `gpt-5.6-sol` | 26.8분 | 62,788 | 4.55M | 10/10 | ✅ 정상 | 43/50 |
| 🥈 `claude-opus-5` | 120분 | 242,674 | 32.2M | 10/10 | ❌ 특정 창 크기에서 3D 씬 소실 | 36/50 |
| 🥉 `grok-4.5` | 10.3분 | 49,027 | 1.04M | 10/10 | ❌ 렌즈플레어 고스트 · 달 명암 반전 | 31/50 |

> **총 토큰 주의**: opus 5의 32.2M 중 98%는 캐시 읽기(153턴 동안 같은 문맥 재독)다.
> 모델이 실제로 생성한 양은 `생성 토큰` 열이며, 그 기준으론 opus가 grok의 5배다(31배 아님).
