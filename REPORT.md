...
| 자동검증 | 10/10 |
| 시각검증 | ✅ 1440×980 · 1456×816 모두 정상, 콘솔 에러 0 |
| 코드 | 3,721줄 (README 산식: public 제외 전체 파일, package-lock 포함) |
| 배포 | https://space3d-fable51-bench.vercel.app |
...
### 관찰
- opus 5 2회차(35.7분 · 55턴)와 fable 5(32.9분 · 82턴)보다 느리고 턴이 많다. 대신 시도 1회에 두 뷰포트 모두 통과했다 — 이 리포트의 5회 중 시각검증까지 통과한 것은 sol 과 fable 5 뿐이었다.
- 세션 로그상 모델이 스스로 헤드리스 Chromium(playwright)을 띄워 로딩·날짜 이동·달 인셋·모바일 레이아웃을 확인했다. §4.3 에서 지적한 "셸 검증은 픽셀을 못 본다"는 한계를 모델이 자체 스크린샷으로 메운 셈이다.
