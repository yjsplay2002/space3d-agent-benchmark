...
| # | 참가자 | 모델 | effort | 시간 | 시도 | 턴 | **생성(출력)** | 총 토큰 | 비용(API 환산) | 자동검증 | 시각검증 | 품질 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 🥇 | codex | `gpt-5.6-sol` | high | **26.8분** | 1 | — | 62,788 | 4,554,421 | N/A | 10/10 | ✅ 정상 | **43/50** |
| 🥈 | claude | `claude-opus-5` | high | 120.0분 | 2 | 153 | 242,674 | 32,150,726 | $25.52 | 10/10 | ❌ 조건부 파손 | 36/50 |
| 🥉 | grok | `grok-4.5` | high | **10.3분** | 1 | 17 | 49,027 | 1,039,396 | $0.73 | 10/10 | ❌ 파손 | 31/50 |
...
- 총 토큰 산식은 CLI마다 다르다. codex는 `input(캐시 포함) + output`, claude/grok은 `input + cache_read + cache_write + output`. **총 토큰은 캐시 읽기가 지배하므로 CLI 간 직접 비교에 부적합하다** — §3 참조. 순위 판단에는 `생성(출력)` 열을 쓸 것.
...
| 참가자 | 신규 입력 | 캐시 읽기 | 캐시 쓰기 | 출력 | 총 토큰 |
|---|---|---|---|---|---|
| gpt-5.6-sol | 193,393 | 4,298,240 | 0 | 62,788 | 4,554,421 |
| claude-opus-5 | 280 | **31,539,590** | 368,182 | 242,674 | 32,150,726 |
| grok-4.5 | 82,465 | 907,904 | — | 49,027 | 1,039,396 |

opus 5의 32.15M 중 **98%가 캐시 읽기**다. 새로 쓴 양이 아니라 153턴을 도는 동안
매 호출마다 같은 문맥을 다시 읽은 누적치다. grok은 17턴이라 캐시 읽기가 90만에
그쳤다 — 턴 수 차이(153 대 17)가 거의 그대로 반영된 값이다.

**따라서 "opus가 grok의 31배를 썼다"는 말은 성립하지 않는다.** 그건 턴 수의
함수이지 작업량의 함수가 아니다.

### 실제 생성량으로 비교

| 참가자 | 출력 토큰 | 산출 코드 | 출력/줄 | 턴 |
|---|---|---|---|---|
| grok-4.5 | 49,027 | 5,078줄 | **9.7** | 17 |
| gpt-5.6-sol | 62,788 | 4,173줄 | 15.0 | — |
| claude-opus-5 | 242,674 | 8,293줄 | 29.3 | 153 |

모델이 실제로 **생성한** 양으로 보면 opus는 grok의 5배다(31배가 아니라).
그 5배 안에는 재시도 2회, 자체 스크린샷 17장 촬영, 후처리 ablation 테스트,
그리고 폐기된 중간 산출물이 포함된다.

grok의 출력/줄 9.7은 셋 중 가장 효율적이다 — 쓴 것을 거의 그대로 남겼다는 뜻이고,
뒤집어 말하면 고쳐 쓰는 과정이 거의 없었다는 뜻이기도 하다(§4.2의 결함과 무관하지 않다).

### 캐시가 비용에 미친 영향

opus의 $25.52는 캐시 읽기 31.5M이 대부분을 차지한다. 캐시 읽기는 신규 입력보다
훨씬 싸지만 31.5M이면 누적된다. 턴 수를 줄이는 것이 비용에 가장 직접적이다.
...
5. **총 토큰은 순위 지표가 아니다.** 캐시 읽기가 지배하고, 캐시 읽기는 턴 수에
   비례한다. 세 CLI의 턴 관리 방식이 달라 이 값으로 "누가 토큰을 아꼈나"를 논하면
   틀린다. 생성(출력) 토큰이 그나마 비교 가능한 값이다.
6. **품질 점수는 Claude가 매겼다.** 블라인드로 진행했지만 자사 모델이 포함된 평가라는 점을 감안해야 한다. 정확성·견고성 점수는 스크린샷이라는 객관 증거에 근거하지만, 코드 품질 점수는 주관이다.
7. **Google Antigravity(gemini)는 불참.** 토큰 사용량을 전혀 보고하지 않아 4대 지표 중 하나를 채울 수 없어 제외했다.
...
## 7. 추가 실행 — claude-fable-5.1 (2026-09-02)

본 리포트의 5회 실행 뒤에 Claude Fable 5.1 로 같은 `PROMPT.md` 를 한 번 더 실행했다.
§1~§6 의 순위·해석에는 반영하지 않았다. 원본 실행과 조건이 다르기 때문이다.

| 항목 | 값 |
|---|---|
| 모델 / effort | `claude-fable-5-1` / high |
| 시간 | 51.2분 (1회 시도, 94턴) |
| 신규 입력 / 캐시 쓰기 / 출력 | 5,114 / 245,193 / 154,024 |
| 캐시 읽기 / 총 토큰 | 7,325,383 / 7,729,714 (비캐시 5.2%) |
| 비용(API 환산) | $18.33 (구독 실행, 실지출 0) |
| 자동검증 | 10/10 |
| 시각검증 | ❌ **실 GPU에서 3D 씬 전체 검정.** SwiftShader 헤드리스에서만 정상 |
| 코드 | 3,721줄 (README 산식: public 제외 전체 파일, package-lock 포함) |
| 배포 | https://space3d-fable51-bench.vercel.app (검은 화면 그대로 배포됨 — 결과물 수정 안 함) |

### 조건 차이
- **하네스 재구성.** 원본 스크립트가 없는 PC라 `harness/run_benchmark.py` 로 다시 썼다. bench.json 의미론(격리 `--safe-mode`, verify 10항목, critical 실패 시 같은 세션 resume 최대 6회, 실패 출력 되먹임, 시도 합산)은 따랐지만 원본과 바이트 단위로 같지는 않다.
- **환경.** Windows 11, claude CLI 2.1.258 (원본 2.1.220), node 22.
- **1차 실행 폐기.** 첫 실행은 20분·20턴에서 claude.ai 월 지출 한도(HTTP 429)로 끊겼다. 하네스가 이를 빌드 실패로 오인해 resume 5회를 소진했고(각 3초, 추가 비용 0), 결과물은 `index.html` 도 없는 미완성이었다. 한도 상향 후 새 세션으로 처음부터 재실행한 값만 표에 넣었다. 폐기분 약 $7.
- **하네스 버그 1건 수정.** Windows 에서 `bash` 가 WSL 로 잡혀 검증이 node 18 로 돌았다. git-bash 를 명시해 고쳤다. 재실행은 수정본으로 돌렸다.
- **품질 점수 없음.** §5 의 블라인드 심사는 하지 않았다.

### 시각검증 상세 — 실 GPU 파손
![fable51 black](docs/screenshots/fable51-gpu-black.jpg)

macOS Chrome 과 Windows RTX 3080(ANGLE D3D11) 모두에서 3D 캔버스가 검고 CSS2D 라벨·달 인셋만 보인다.
GL 에러도 콘솔 에러도 없다. 패스를 하나씩 빼며 이등분한 결과:

| 조건 | 결과 |
|---|---|
| 그대로 | 검정 |
| 블룸 패스 제거 | 정상 (아래) |
| 플레어 / 그레인 제거, 블룸 강도 0, 블룸을 플레어 뒤로 | 검정 |
| 렌더타깃 MSAA 끔 (samples 0) | 정상 |
| 렌더타깃 UnsignedByte | 정상 |
| 렌더타깃 Float | 검정 |

![fable51 nobloom](docs/screenshots/fable51-gpu-nobloom.jpg)

즉 `UnrealBloomPass` + HalfFloat/Float + MSAA 렌더타깃 조합이 실 GPU 드라이버에서 프레임을 통째로 날린다.
SwiftShader 는 이 경로를 정상 처리해서 헤드리스 검증으로는 잡히지 않는다. 같은 조합을 쓴 `fable51-v1spec/` 은
실 GPU 에서 렌더링되므로 정확한 트리거는 빌드 내부 어딘가에 더 있다 — 여기서는 더 파지 않았다.

### 관찰
- opus 5 2회차(35.7분 · 55턴)와 fable 5(32.9분 · 82턴)보다 느리고 턴이 많다.
- 세션 로그상 모델이 스스로 헤드리스 Chromium(playwright)을 띄워 로딩·날짜 이동·달 인셋·모바일 레이아웃을 확인했다. 그런데 그 Chromium 도 SwiftShader 였다. **모델이 픽셀을 봤는데도 실 GPU 결함을 못 잡았다** — §4.3 의 결론("셸 검증은 픽셀을 못 본다")에 "소프트웨어 렌더러의 픽셀도 실기기와 다르다"를 보태야 한다. 이 리포트 작성자도 처음엔 같은 SwiftShader 스크린샷을 보고 정상으로 기록했다가 실기기에서 정정했다.
- 결과적으로 opus-5 두 실행과 같은 부류다: 자동검증 만점, 브라우저에서 파손. 6회 실행 중 실기기까지 통과한 것은 sol 과 fable 5 둘뿐이다.
- 출력/줄 41.4 는 표 중 가장 높다. 코드는 짧고(3,721줄) 출력 토큰은 fable 5 보다 많다. 자체 검증에 쓴 토큰이 상당 부분이다.
